...
public class GridAlg {
	...
	public void initaliseDensities() {
		...
		for (int i = 0; i < getAnimals().length; i++) {
			...
			for (int x = 1; x < getNeighbours().length - 1; x++) {			
				for (int y = 1; y < getNeighbours()[0].length - 1; y++) {				
					...
					getAnimals()[i].setDensity(x, y, randomValue);
					getAnimals()[i].setNextDensity(x, y, randomValue);
				}
			}
		}
	}
	...
	public void syncUpdate() {
		...
		for (int i = 0; i < animals.length; i++) {
			...
			for (int x = 1; x < (getNeighbours().length-1); x++) {
				for (int y = 1; y < (getNeighbours()[0].length-1); y++) {
					if (getNeighbours(x, y) != -1) {
						animals[i].calcNextDensity(x, y, dt, animals,
								neighbours[x][y]);
